...
or_ci = cbind(summary(housing_fit_nom)$coefficients, summary(housing_fit_nom)$standard.errors) %>% 
  ...
  select(model, exp_contact_low_ci, exp_home_type_house_ci, exp_home_type_tower_block)

or_ci %>% 
  filter(model == "sat_med") %>% 
  select(exp_contact_low_ci)
  
```

The odds ratio (with 95% CI) between medium satisfaction and low satisfaction for:

* degree of contact low vs high is `r or_ci %>% filter(model == "sat_med") %>% select(exp_contact_low_ci)`

* home type house vs home type apartment is `r or_ci %>% filter(model == "sat_med") %>% select(exp_home_type_house_ci)`

* home type tower block vs home type aparment is `r or_ci %>% filter(model == "sat_med") %>% select(exp_home_type_tower_block)`

The odds ratio (with 95% CI) between high satisfaction and low satisfaction for:

* degree of contact low vs high is `r or_ci %>% filter(model == "sat_high") %>% select(exp_contact_low_ci)`

* home type house vs home type apartment is `r or_ci %>% filter(model == "sat_high") %>% select(exp_home_type_house_ci)`

* home type tower block vs home type aparment is `r or_ci %>% filter(model == "sat_high") %>% select(exp_home_type_tower_block)`